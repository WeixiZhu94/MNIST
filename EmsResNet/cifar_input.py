...
def build_input(dataset, batch_size, mode):
  ...
  if mode == 'train':
    ...
    example_queue = tf.RandomShuffleQueue(
        ...
        seed=7)
    #0,1,8,9
    queue_cat_0 = tf.RandomShuffleQueue(
        capacity=16 * batch_size,
        min_after_dequeue=8 * batch_size,
        dtypes=[tf.float32, tf.int32],
        shapes=[[image_size, image_size, depth], [1]],
        seed=8)
    #2,3,4,5,6,7
    queue_cat_1 = tf.RandomShuffleQueue(
        capacity=16 * batch_size,
        min_after_dequeue=8 * batch_size,
        dtypes=[tf.float32, tf.int32],
        shapes=[[image_size, image_size, depth], [1]],
    ...
    example_queue = tf.FIFOQueue(
        ...
        shapes=[[image_size, image_size, depth], [1]])
    #0,1,8,9
    queue_cat_0 = tf.FIFOQueue(
        3 * batch_size,
        dtypes=[tf.float32, tf.int32],
        shapes=[[image_size, image_size, depth], [1]])
    #2,3,4,5,6,7
    queue_cat_1 = tf.FIFOQueue(
        3 * batch_size,
        dtypes=[tf.float32, tf.int32],
        shapes=[[image_size, image_size, depth], [1]])
    num_threads = 1
  ...
  images_cat_0, labels_cat_0 = queue_cat_0.dequeue_many(batch_size)
  images_cat_1, labels_cat_1 = queue_cat_1.dequeue_many(batch_size)
  ...
  return images, labels, images_cat_0, labels_cat_0, images_cat_1, labels_cat_1
